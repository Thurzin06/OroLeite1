...
function login(email, password) {
  ...
  if (userByEmail.password !== password)
    return { ok: false, msg: "Senha incorreta.", code: "wrong_password" };
...
function logout() {
  ...
  try {
    if (window.refreshProductPrices) window.refreshProductPrices();
  } catch (e) {}
}
  ...
  if (signUpForm) {
    ...
    signUpForm.addEventListener("submit", (ev) => {
      ...
      if (!validateSignupEmail()) {
        if (signupFeedback)
          signupFeedback.textContent = "Insira um e-mail válido.";
      ...
      if (!res.ok) {
        if (signupFeedback)
          signupFeedback.textContent = res.msg || "Erro ao cadastrar.";
  ...
  if (signInForm) {
    signInForm.addEventListener("submit", (ev) => {
      try {
        ...
        const password =
          (document.getElementById("signinPassword") || { value: "" }).value ||
          "";
        ...
        if (!res.ok) {
          ...
          if (loginFeedback)
            loginFeedback.textContent = res.msg || "Credenciais inválidas.";
        ...
        try {
          showToast("Erro inesperado. Veja o console.", "error");
        } catch (e) {}
      }
    });
  }
...
function setupGoogleSignIn() {
  const googleButtons =
    Array.from(document.querySelectorAll(".google-btn")) || [];
  ...
  async function doGoogleSignIn(ev) {
    ...
    if (!window.firebase || !window.FIREBASE_CONFIG) {
      setLoading(false);
      showToast(
        "Firebase não está configurado. Cole o config em login.html",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          name,
          lastLogin: new Date().toISOString(),
          loginCount: u && u.loginCount ? u.loginCount : 1,
          avatar,
        })
      );
      ...
      if (err && err.code === "auth/unauthorized-domain") {
        ...
        console.warn(
          "[google-auth] origem detectada:",
          location.origin,
          "hostname:",
          location.hostname
        );
        showToast(
          "Este domínio não está autorizado no Firebase. Vá em Firebase Console → Authentication → Authorized domains e adicione: " +
            detected,
          "error",
          9000
        );
        ...
        showToast(
          "Erro no login com Google: " + (err.message || err.code || ""),
          "error"
        );
      }
    } finally {
  ...
  if (!window.firebase || !window.FIREBASE_CONFIG) {
    if (hintEl)
      hintEl.textContent =
        "Google sign-in não configurado. Cole o config do Firebase em login.html.";
    console.warn(
      "[google-auth] Firebase não configurado (window.FIREBASE_CONFIG ausente)"
    );
  } else {
    if (hintEl) hintEl.textContent = "Clique no ícone do Google para entrar.";
    console.info(
      "[google-auth] Firebase configurado. Pronto para Google sign-in."
    );
  }
  ...
  if (googleButtons && googleButtons.length) {
    googleButtons.forEach((b) => {
      ...
      b.addEventListener("click", (ev) => {
        console.log(
          "[google-auth] google button clicked",
          b.dataset && b.dataset.role
        );
        doGoogleSignIn(ev);
      });
    });
